...
class ZQTrendStrategy2:
    ...
    # 回测窗口参数
    parameters = ["tar_bar_range", "tar_hor_range", "tar_hor_size"]
    ...
    def get_horizontal(self):
        ...
        if hor_range > self.tar_hor_range:
            self.exist_hor = False
            return
        if self.lower_rail <= self.curr_bar.close_price <= self.upper_rail:
            self.exist_hor = False
            return
        ...
        print(f'区间开始时间:{self.hor_bars[0].local_datetime}, 上轨: {self.upper_rail}, 下轨: {self.lower_rail}, 幅度: {round(hor_range, 3)}%')
