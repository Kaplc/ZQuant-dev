...
class VVD(ScriptBase):
    ...
    def __init__(self, symbol: str, exchange: Exchange, zq_interval: str, start: datetime, end: datetime):
        super().__init__(symbol, exchange, zq_interval, start, end)
    ...
    def run(self):
        ...
        for bar in self.bars:
            ...
            if price_region_start == price_region_end:
                price_region_dict[
                    str(int(price_region_start)) + '-' + str(int(price_region_start + step))] += bar.volume
            else:
                # 两头未填满价格区间的价格范围
                ...
                price_region_dict[
                    str(int(price_region_end)) + '-' + str(int(price_region_end + step))] += end_region_vol

        vol_ma = VolMa(price_region_dict)

        print('生成柱状图.')
        price_dict = {k: v for k, v in price_region_dict.items() if v != 0 and v >= vol_ma}
        ...
        save_path = f"/home/kaplc/PycharmProjects/ZQuant_dev/apps/vnpy_ctastrategy/strategies/script/Data/VVD/{self.start.strftime('%Y-%m-%d')}~{self.end.strftime('%Y-%m-%d')}.png"
